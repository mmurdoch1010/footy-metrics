...
def get_Command_Line_Args():
    parser = argparse.ArgumentParser(description='A program for generating projected standings for soccer leagues, using elo and monte carlo simulations.')
...
def monte_Carlo(team, n, v):
    ...
    for opponent in fixtures[team]:
        match fixtures[team][opponent]:
            case 1:
                g.append(elo_Win_Prob(elo[team], elo[opponent]))
            case 0:
                g.append(elo_Win_Prob(elo[team], elo[opponent]))
                g.append(elo_Win_Prob(elo[team], elo[opponent]))
    for i in range(t + 1):
...
def main():
    ...
    sorted_Standings = sorted(metric_Standings.items(), key=lambda x : x[1], reverse=True)
    for team in sorted_Standings:
        print(f"{team[0]}: {team[1]}")
    
    if args.write:
        write_Results(sorted_Standings)
